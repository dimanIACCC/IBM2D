...
SolidBody::SolidBody(double x, double y, double ux, double uy, double omega, double rho, int Nn, bool moving, int &name)
{
	...
	this->moving = moving;
	this->name = name;
	this->omega_new = this->omega_n;
	this->uc_new    = this->uc_n;
	this->xc_new    = this->xc_n;
	this->Fr = 0.;
	this->n_moving = 0;
}
			...
			if (line == "circle{") {
				...
				while (line != "}") {
					...
					if (VALUE.size() > 0) {
						...
						else if (PAR == "omega_new")  omega        = stod(VALUE);
	...
	if (   (par.N_step - par.AddSolids_start) % par.AddSolids_interval == 0   &&   (par.N_step >= par.AddSolids_start)) { //create new solids starting from $AddSolids_start$ iteration with interval of $AddSolids_interval$ iterations
		for (int i = 0; i < par.AddSolids_N; i++) { // add $AddSolids_N$ solids
			...
			for (auto solid = Solids.begin(); solid != Solids.end(); solid++) {
				if (length(x - solid->xc_new) < par.k_dist * (par.r + solid->r)) {
					add = false;
					break;
				}
			}
...
bool Collide(Circle& s1, Circle& s2, Param par) {
	...
	if (distance <= par.k_dist*(s1.r + s2.r)) {
		...
		if (par.InelasticCollision) { //Perfectly inelastic collision
			s1.uc_new = (s1.uc_new + s2.uc_new) / 2;
			s2.uc_new = s1.uc_new;
		}
		else { //Perfectly elastic collision
			r = r / distance;
			double u1_before = dot_product(s1.uc_new, r);
			double u2_before = dot_product(s2.uc_new, r);
			if (u1_before - u2_before < 0.0) { // if Solids move to each other
				...
				s1.uc_new += (u1_after - u1_before) * r;
				s2.uc_new += (u2_after - u2_before) * r;
			}
		}
	}
	return result;
...
void Solids_move(std::list<Circle> &solidList, Param par) {
	...
	for (auto one = solidList.begin(); one != solidList.end(); one++) {
		double DistUpper = par.H - one->xc_new[2];//<----distance to upper wall
		double DistLower = one->xc_new[2];//<-------distance to lower wall
		if ((DistUpper < par.k_dist * one->r && one->uc_new[2] > 0) ||
		    (DistLower < par.k_dist * one->r && one->uc_new[2] < 0)) {
			if (Debug) std::cout << "Collision with wall detected" << std::endl;
			one->uc_new[2] = -one->uc_new[2];
		}
	}
...
void Solids_velocity_new(std::list<Circle>& Solids, Param par) {
	for (auto& it : Solids) {
		if (it.moving) {
			...
			it.xc_new = it.xc_n + 0.5 * (it.uc + it.uc_new) * par.d_t;
			for (size_t k = 0; k < it.Nn; ++k) {
				it.Nodes[k].x = rotate_Vector_around_vector(it.Nodes[k].xn, 0.5 * (it.omega + it.omega_new) * par.d_t); //rotate
			}
...
void Circle::integrals(Matrix U_n, Matrix V_n, Matrix U_new, Matrix V_new, Param par) {
	...
	for (int i = i_min; i <= i_max; ++i) {
		for (int j = j_min; j <= j_max; ++j) {
			...
			unew[2] = 0.0;
			unew[3] = 0.0;
			integralV_unew_r += Frac * x_product(xu - xc_new, unew);
		}
	}
	...
	for (int i = i_min; i <= i_max; ++i) {
		for (int j = j_min; j <= j_max; ++j) {
			...
			vnew[3] = 0.0;
			integralV_unew_r += Frac * x_product(xv - xc_new, vnew);
		}
	}
